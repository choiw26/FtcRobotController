...
@Autonomous
public class OpMode_SquareAutomation_2 extends OpMode{
    enum State{
        MOVE,
        SPIN
    ...
    public void loop () {
        ...
        telemetry.addData("Distance right (CM)", board.getDistance1(DistanceUnit.CM));
        telemetry.addData("Distance left (CM)", board.getDistance2(DistanceUnit.CM));
        ...
        switch (state) {
            ...
            case SPIN:
                ...
                if (Math.abs(botHeading) >= Math.PI / 2 || (board.getDistance1(DistanceUnit.CM)
                        >= 150.0 || board.getDistance2(DistanceUnit.CM) >= 150.0)) {
                    state = State.MOVE;
                }
        }
    }
